...
class AccountInvoiceLine(models.Model):
    ...
    # [Start] Onchange function for updating tax_ids based on qbd_tax_code selection or clear if not applicable #####################
    @api.onchange("qbd_tax_code")
    def _onchange_qbd_tax_code(self):
        # print("\n\n _onchange_qbd_tax_code function triggered ----- \n\n ")
        for line in self:
            # print(f"\n\n qbd_tax_code ----- {line.qbd_tax_code.display_name if line.qbd_tax_code else 'None'} \n\n")

            # CASE 1: If qbd_tax_code is cleared → clear tax_ids #################
            if not line.qbd_tax_code:
                # print("\n\n qbd_tax_code is None → Clearing tax_ids \n\n")
                line.tax_ids = [(5, 0, 0)]  # clear taxes
                continue

            # CASE 2: If qbd_tax_code is 'Tax' ###################################
            if line.qbd_tax_code.display_name.lower() == "tax":
                qbd_tax = line.move_id.qbd_tax
                # print(f"\n\n qbd_tax (from invoice) ----- {qbd_tax} \n\n")

                if qbd_tax:
                    line.tax_ids = [(6, 0, [qbd_tax.id])]
                    # print(f"\n\n tax_ids updated with qbd_tax ----- {qbd_tax.id} \n\n")
                else:
                    # print("\n\n No qbd_tax found on invoice → Clearing taxes \n\n")
                    line.tax_ids = [(5, 0, 0)]

            # CASE 3: If qbd_tax_code is not 'Tax' ###############################
            else:
                # print(f"\n\n qbd_tax_code is {line.qbd_tax_code.display_name}, not 'Tax' → Clearing tax_ids \n\n")
                line.tax_ids = [(5, 0, 0)]
    # [End] Onchange function for updating tax_ids based on qbd_tax_code selection or clear if not applicable #######################
...
class AccountInvoice(models.Model):
    ...
    def create_invoice(self, invoices):
        _logger.info(f"\n\n\n create_invoice function triggered ----- {format(invoices)}\n\n ")
                ...
                if 'quickbooks_id' in invoice and invoice.get('quickbooks_id'):
                    ...
                    if not invoice_id:
                        ...
                        if vals:
                            ...
                            if invoice.get('state') and invoice.get('state') == 'paid':
            ...
            if 'quickbooks_id' in bill and bill.get('quickbooks_id'):
                ...
                if not vendor_bill_id:
                    ...
                    if vals:
                        ...
                        if bill.get('state') and bill.get('state') == 'paid':
                            new_invoice_id.sudo().action_post()
    ...
    def _prepare_invoice_dict(self, invoice, is_vendor_bill = None):
        ...
        if invoice.get('tax_qbd_id'):
            tax_id = self.env['account.tax'].search([('quickbooks_id','=',invoice.get('tax_qbd_id'))])
...
# [Start] Function for creating invoice lines ####################################################################
    def create_invoice_lines(self, invoice_lines, invoice_id):
        invoice_line_id_list = []
        company = self.env['res.users'].browse(2).company_id

        for line in invoice_lines:
            _logger.info(_('\n\nProcessing line : %s' % line))
            vals_ol = {'move_id': invoice_id.id}

            # Product ################################################################
            product = self.env['product.product'].search(
                [('quickbooks_id', '=', line.get('product_name'))], limit=1)
            if not product:
                continue
            vals_ol['product_id'] = product.id

            # Tax code reference #####################################################
            tax_code = False
            if line.get('tax_code'):
                tax_code = self.env['qbd.tax.code'].search(
                    [('name', '=', line['tax_code'])], limit=1)
                if tax_code:
                    vals_ol['qbd_tax_code'] = tax_code.id

            # Tax from QuickBooks (apply only if taxable) #############################
            if line.get('tax_qbd_id') and tax_code and tax_code.name.lower() != "non":
                tax_id = self.env['account.tax'].search(
                    [('quickbooks_id', '=', line['tax_qbd_id'])], limit=1)
                if tax_id:
                    invoice_id.write({'qbd_tax': tax_id.id})
                    vals_ol['tax_ids'] = [(6, 0, [tax_id.id])]
            else:
                vals_ol['tax_ids'] = [(5, 0, 0)]  # Clear any tax if Non

            # Account #################################################################
            vals_ol['account_id'] = (
                product.property_account_income_id.id
                or product.categ_id.property_account_income_categ_id.id
                or company.qb_income_account.id
            )

            # Quantity & price #########################################################
            vals_ol['quantity'] = float(line.get('quantity') or 0.0)
            vals_ol['price_unit'] = float(line.get('price_unit') or 0.0)
            vals_ol['name'] = self.cleanup_productdesc(line.get('name') or '')

            # Create OL line only ######################################################
            line_rec = self.env['account.move.line'].create(vals_ol)
            invoice_line_id_list.append(line_rec.id)

        return invoice_line_id_list
# [End] Function for creating invoice lines ######################################################################
    ...
    def export_invoices(self, invoice_lst=False, is_vendor_bill=False):
        ...
        if invoices:
            for invoice in invoices:
                ...
                if company.export_updated_record:
                    if self._context.get('is_vendor_bill'):
                        invoice_dict = self.get_invoice_dict(invoice, company.export_updated_record, is_vendor_bill = True)
                    else:
    ...
    def export_invoice_to_qbd_server_action(self):
        ...
        if all([x.move_type == 'in_invoice' for x in self]):
            ...
            # print("Errrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrooooooooooooooooooooooooeeeeeeeeeeeeeeeerrrrrrrr")
        elif all([x.move_type == 'out_invoice' for x in self]):
            if self.invoice_date:
                self.export_invoices(invoice_lst=self, is_vendor_bill=False)
                # print("****************************************invoiceErrorrrrrrrrrrrrrrrrrrrrrr********************")
            else:
                raise UserError(
