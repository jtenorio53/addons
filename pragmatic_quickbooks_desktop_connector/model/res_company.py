...
class Qb_Config_Settings(models.Model):
    ...
    qb_account_recievable = fields.Many2one('account.account', 'Account Receivable')
    ...
    def check_duplicates(self):
        ...
        if not self._context.get('partner_only') and not self._context.get('vendor_only') and not self._context.get('product_only'):
        ...
        if called_from_button or self._context.get('partner_only'):
            self.env.cr.execute(
                "SELECT id,parent_id,name,display_name,quickbooks_id FROM res_partner WHERE supplier_rank= 0 and display_name IN (SELECT display_name FROM res_partner WHERE supplier_rank= 0 GROUP BY display_name HAVING (COUNT(display_name) > 1))")
            result = self.env.cr.fetchall()
            if result:
                for row in result:
                    if row[1]:
...
###########  Import ###############
    ...
    def import_sales_tax(self, logger_obj):
        try:
            ...
            if sales_tax_data:
                data = json.loads(sales_tax_data)
    ...
    def import_product(self):
        ...
        if not self.last_imported_qbd_id_for_product:
            ...
            formatted_data = json.loads(response.text)
            # print("\n\n import product after response+++++++++++++++++++++++++++++++++++++++++++++++++++++++", response ,"              ",formatted_data)
...
#         print('\n\nRequest !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1 \n\n')
        ...
        _logger.info("RESPONSE===>>>>>>{}".format(response.text))
        payment_method_data = json.loads(response.text)
...
#         print("\n\n in invoice method")
        ...
        if not self.last_imported_qbd_id_for_invoice:
            ...
            formatted_data = json.loads(response.text)
            # print("===222====formatted_data===============",formatted_data)
    ...
    def import_vendorbill_payments(self):
        ...
        try:
            ...
            for payment in parsed_response:
                # print("\n\n\n========payment=======", json.dumps(payment, indent=3))
                        ...
                        if payment.get('invoice_data'):
                            ...
                            if 'qbo_partner_id' in payment and payment.get('qbo_partner_id'):
                                ...
                                if partner_id:
                                    ...
                                    imported_count += 1
            if imported_count > 0:
                return self.sendMessage({'Message': f"{imported_count} Vendor Payment(s) imported successfully!"})
            else:
                return self.sendMessage({'Message': "No new Vendor Payments found to import."})

        except Exception as ex:
            _logger.warning(ex)
            return self.sendMessage({'Message': f"Error occurred while importing payments: {str(ex)}"})
