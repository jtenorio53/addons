...
# [Start] Function for exporting invoices from Odoo to QBD & than QBD to Odoo ################################################################
@invoice.route('/QBD/export_invoices', methods=['POST'])
def export_TPA_invoice_to_QBD():
    ...
    # Get request data from client (expects JSON with 'invoices_list') #############################
    req = request.get_json(force=True)
    # print(f"\n\n request_json ----- {req} \n\n")

    # [Start] Helper function to escape single quotes ##############################################
    def sanitize(v):
        if v is None:
            return ''
        if isinstance(v, str):
            return v.replace("'", "''")
        return v
    # [End] Helper function to escape single quotes ################################################

    # Process invoices if present in request #######################################################
    if 'invoices_list' in req:
        tpa_invoice_list = []
        ...
        for rec in req.get('invoices_list'):
            ...
            # Extract and sanitize fields from invoice ##############################################
            qbd_memo = sanitize(rec.get('qbd_memo'))
            is_vendor_bill = rec.get('is_vendor_bill')

            ref_number = rec.get('qbd_number')
            if ref_number is not None and str(ref_number).strip() == '':
                ref_number = None

            # print(f"\n\n qbd_number(ref_number) ----- {ref_number} \n\n")

            default_tax_on_invoice = sanitize(rec.get('default_tax_on_invoice'))

            # Check if we're updating an existing QBD record or not ##################################
            if rec.get('invoice_order_qbd_id'):
                quickbooks_id = rec.get('invoice_order_qbd_id')
                is_update = 1
            else:
            ...
            # Check if invoice with RefNumber already exists in QBD #################################
            if ref_number:
                if is_vendor_bill:
                    check_record = "Select Top 1 TxnId,RefNumber,TimeModified from Bill where RefNumber='{}'".format(sanitize(ref_number))
                else:
                    check_record = "Select Top 1 TxnId,RefNumber,TimeModified from Invoice where RefNumber='{}'".format(sanitize(ref_number))
            ...
            for lines in rec.get('invoice_lines', []):
                count += 1
                product_quickbooks_id = sanitize(lines.get('product_name'))

                invoice_line_quantity = float(lines.get('quantity') or 0.0)
                invoice_line_rate = float(lines.get('price_unit') or 0.0)
                invoice_line_amount = float(lines.get('price_subtotal') or 0.0)
                invoice_line_desc = sanitize(lines.get('name'))
                customer_quickbooks_id = sanitize(rec.get('partner_name'))

                # Format transaction date for QBD SQL ###############################################
                invoice_line_txn_date_str = rec.get('invoice_date') or ''
                invoice_line_txn_date = "{d'" + invoice_line_txn_date_str + "'}" if invoice_line_txn_date_str else "NULL"
                # print(f"\n\n invoice_line_txn_date ----- {invoice_line_txn_date} \n\n")

                # Handle SaveToCache flag: only 0 for final (last) line, 1 for interim lines ########
                is_last_line = (count == len(rec.get('invoice_lines', [])))
                save_to_cache = 0 if is_last_line else 1


                # Other fields ####################################################################
                tax_id = sanitize(lines.get('tax_id') or '')
                qbd_tax_code = sanitize(lines.get('qbd_tax_code') or '')
                payment_terms = sanitize(lines.get('payment_terms') or '')

                # INSERT Logic (New Invoice/Bill) #######################################################################
                if not is_update:
                     # If invoice already exists, skip insert ################################################
                    if ref_number and is_present:
                        # print("\n\n Found existing by RefNumber; mapping back QBD id to Odoo \n\n")
                        tpa_invoice_dict['odoo_id'] = rec.get('odoo_id')
                        tpa_invoice_dict['quickbooks_id'] = is_present[0]
                        tpa_invoice_dict['message'] = 'Quickbooks Id Successfully Added'
                        tpa_invoice_dict['qbd_invoice_ref_no'] = is_present[1]
                    ...
                    # Build dynamic insert query (Invoice or Bill) ####################################################
                    if is_vendor_bill:
                        # Create BillItemLine insert query (with or without payment_terms & tax_id) ##########
                        if tax_id:
                            if payment_terms:
                                if ref_number:
                                    insertQuery = "Insert into BillItemLine (VendorRefListID, RefNumber, ItemLineItemRefListID, ItemLineDesc, ItemLineCost, ItemLineAmount, FQSaveToCache, Memo, TxnDate, ItemLineQuantity, TermsRefFullName) VALUES ('{}','{}','{}','{}',{},{},{},'{}',{}, {}, '{}')".format(customer_quickbooks_id, sanitize(ref_number), product_quickbooks_id, invoice_line_desc, invoice_line_rate, invoice_line_amount, save_to_cache, qbd_memo, invoice_line_txn_date, invoice_line_quantity, payment_terms)
                                else:
                                    insertQuery = "Insert into BillItemLine (VendorRefListID, ItemLineItemRefListID, ItemLineDesc, ItemLineCost, ItemLineAmount, FQSaveToCache, Memo, TxnDate, ItemLineQuantity, TermsRefFullName) VALUES ('{}','{}','{}',{},{},{},'{}',{}, {}, '{}')".format(customer_quickbooks_id, product_quickbooks_id, invoice_line_desc, invoice_line_rate, invoice_line_amount, save_to_cache, qbd_memo, invoice_line_txn_date, invoice_line_quantity, payment_terms)
                            else:
                                if ref_number:
                                    insertQuery = "Insert into BillItemLine (VendorRefListID, RefNumber, ItemLineItemRefListID, ItemLineDesc, ItemLineCost, ItemLineAmount, FQSaveToCache, Memo, TxnDate, ItemLineQuantity) VALUES ('{}','{}','{}','{}',{},{},{},'{}',{}, {})".format(customer_quickbooks_id, sanitize(ref_number), product_quickbooks_id, invoice_line_desc, invoice_line_rate, invoice_line_amount, save_to_cache, qbd_memo, invoice_line_txn_date, invoice_line_quantity)
                                else:
                                    insertQuery = "Insert into BillItemLine (VendorRefListID, ItemLineItemRefListID, ItemLineDesc, ItemLineCost, ItemLineAmount, FQSaveToCache, Memo, TxnDate, ItemLineQuantity) VALUES ('{}','{}','{}',{},{},{},'{}',{}, {})".format(customer_quickbooks_id, product_quickbooks_id, invoice_line_desc, invoice_line_rate, invoice_line_amount, save_to_cache, qbd_memo, invoice_line_txn_date, invoice_line_quantity)
                        else:
                            if payment_terms:
                                if ref_number:
                                    insertQuery = "Insert into BillItemLine (VendorRefListID, RefNumber, ItemLineItemRefListID, ItemLineDesc, ItemLineCost, ItemLineAmount, FQSaveToCache, Memo, TxnDate, ItemLineQuantity, TermsRefFullName) VALUES ('{}','{}','{}','{}',{},{},{},'{}',{}, {}, '{}')".format(customer_quickbooks_id, sanitize(ref_number), product_quickbooks_id, invoice_line_desc, invoice_line_rate, invoice_line_amount, save_to_cache, qbd_memo, invoice_line_txn_date, invoice_line_quantity, payment_terms)
                        ...
                        # Create InvoiceLine insert query (with or without payment_terms & tax_id) ###########
                        if tax_id:
                            if payment_terms:
                                if ref_number:
                                    if is_last_line and default_tax_on_invoice:
                                        insertQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, ItemSalesTaxRefListID, InvoiceLineTaxCodeRefFullName, Memo, RefNumber, TermsRefFullName, ItemSalesTaxRefFullName) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}','{}','{}', '{}', '{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, tax_id, qbd_tax_code, qbd_memo, sanitize(ref_number), payment_terms, default_tax_on_invoice)
                                    else:
                                        insertQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, ItemSalesTaxRefListID, InvoiceLineTaxCodeRefFullName, Memo, RefNumber, TermsRefFullName) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}','{}','{}', '{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, tax_id, qbd_tax_code, qbd_memo, sanitize(ref_number), payment_terms)
                                else:
                                    if is_last_line and default_tax_on_invoice:
                                        insertQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, ItemSalesTaxRefListID, InvoiceLineTaxCodeRefFullName, Memo, TermsRefFullName, ItemSalesTaxRefFullName) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}','{}','{}', '{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, tax_id, qbd_tax_code, qbd_memo, payment_terms, default_tax_on_invoice)
                                    else:
                                        insertQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, ItemSalesTaxRefListID, InvoiceLineTaxCodeRefFullName, Memo, TermsRefFullName) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}','{}','{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, tax_id, qbd_tax_code, qbd_memo, payment_terms)
                            else:
                                if ref_number:
                                    if is_last_line and default_tax_on_invoice:
                                        insertQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, ItemSalesTaxRefListID, InvoiceLineTaxCodeRefFullName, Memo, RefNumber, ItemSalesTaxRefFullName) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}', '{}', '{}', '{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, tax_id, qbd_tax_code, qbd_memo, sanitize(ref_number), default_tax_on_invoice)
                                    else:
                                        insertQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, ItemSalesTaxRefListID, InvoiceLineTaxCodeRefFullName, Memo, RefNumber) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}', '{}', '{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, tax_id, qbd_tax_code, qbd_memo, sanitize(ref_number))
                                else:
                                    if is_last_line and default_tax_on_invoice:
                                        insertQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, ItemSalesTaxRefListID, InvoiceLineTaxCodeRefFullName, Memo, ItemSalesTaxRefFullName) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}','{}','{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, tax_id, qbd_tax_code, qbd_memo, default_tax_on_invoice)
                                    else:
                            ...
                            if payment_terms:
                                if ref_number:
                                    if is_last_line and default_tax_on_invoice:
                                        insertQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, InvoiceLineTaxCodeRefFullName, Memo, RefNumber, TermsRefFullName, ItemSalesTaxRefFullName) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}', '{}', '{}', '{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, qbd_tax_code, qbd_memo, sanitize(ref_number), payment_terms, default_tax_on_invoice)
                                    else:
                                        insertQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, InvoiceLineTaxCodeRefFullName, Memo, RefNumber, TermsRefFullName) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}', '{}', '{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, qbd_tax_code, qbd_memo, sanitize(ref_number), payment_terms)
                                else:
                                    if is_last_line and default_tax_on_invoice:
                                        insertQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, InvoiceLineTaxCodeRefFullName, Memo, TermsRefFullName, ItemSalesTaxRefFullName) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}', '{}', '{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, qbd_tax_code, qbd_memo, payment_terms, default_tax_on_invoice)
                                    else:
                                        insertQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, InvoiceLineTaxCodeRefFullName, Memo, TermsRefFullName) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}', '{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, qbd_tax_code, qbd_memo, payment_terms)
                            else:
                                if ref_number:
                                    if is_last_line and default_tax_on_invoice:
                                        insertQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, InvoiceLineTaxCodeRefFullName, Memo, RefNumber, ItemSalesTaxRefFullName) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}', '{}', '{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, qbd_tax_code, qbd_memo, sanitize(ref_number), default_tax_on_invoice)
                                    else:
                                        insertQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, InvoiceLineTaxCodeRefFullName, Memo, RefNumber) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}', '{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, qbd_tax_code, qbd_memo, sanitize(ref_number))
                                else:
                                    if is_last_line and default_tax_on_invoice:
                                        insertQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, InvoiceLineTaxCodeRefFullName, Memo, ItemSalesTaxRefFullName) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}', '{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, qbd_tax_code, qbd_memo, default_tax_on_invoice)
                                    else:
                ...
                # UPDATE Logic (Existing Invoice/Bill) ################################################################
                else:
                    if is_vendor_bill:
                        check_line_present = "SELECT ItemLineItemRefListID FROM BillItemLine where TxnId='{}' and ItemLineItemRefListID='{}'".format(quickbooks_id, product_quickbooks_id)
                    else:
                    ...
                    # print(f"\n\n check_line_present ----- {check_line_present} \n\n")
                    cursor.execute(check_line_present)
                    record = cursor.fetchone()
                    chk_bool = record[0] if record else None
                    # print(f"\n\n line_exists(chk_bool) ----- {chk_bool} \n\n")

                    if chk_bool:
                        # Perform update on existing line #########################################
                        if is_vendor_bill:
                            updateQuery = "Update BillItemLine set ItemLineQuantity={}, ItemLineCost={}, ItemLineAmount={} where ItemLineItemRefListID='{}' and VendorRefListID='{}' and TxnDate={} and TxnId='{}'".format(invoice_line_quantity, invoice_line_rate, invoice_line_amount, product_quickbooks_id, customer_quickbooks_id, invoice_line_txn_date, quickbooks_id)
                        else:
                            updateQuery = "Update InvoiceLine Set InvoiceLineQuantity= {}, InvoiceLineRate={}, InvoiceLineAmount={} where InvoiceLineItemRefListID='{}' and CustomerRefListID='{}' and TxnDate={} and TxnId='{}'".format(invoice_line_quantity, invoice_line_rate, invoice_line_amount, product_quickbooks_id, customer_quickbooks_id, invoice_line_txn_date, quickbooks_id)
                    else:
                        # Insert new line during update ##########################################
                        if is_vendor_bill:
                            save_to_cache2 = 0  # keep your earlier special case #################
                            if tax_id:
                                if payment_terms:
                                    updateQuery = "Insert into BillItemLine (VendorRefListID, RefNumber, ItemLineItemRefListID, ItemLineDesc, ItemLineCost, ItemLineAmount, FQSaveToCache, Memo, TxnDate, ItemLineQuantity) VALUES ('{}','{}','{}','{}',{},{},{},'{}',{}, {})".format(customer_quickbooks_id, sanitize(ref_number) if ref_number else '', product_quickbooks_id, invoice_line_desc, invoice_line_rate, invoice_line_amount, save_to_cache2, qbd_memo, invoice_line_txn_date, invoice_line_quantity)
                                else:
                                    updateQuery = "Insert into BillItemLine (VendorRefListID, RefNumber, ItemLineItemRefListID, ItemLineDesc, ItemLineCost, ItemLineAmount, FQSaveToCache, Memo, TxnDate, ItemLineQuantity) VALUES ('{}','{}','{}','{}',{},{},{},'{}',{}, {})".format(customer_quickbooks_id, sanitize(ref_number) if ref_number else '', product_quickbooks_id, invoice_line_desc, invoice_line_rate, invoice_line_amount, save_to_cache2, qbd_memo, invoice_line_txn_date, invoice_line_quantity)
                        else:
                            if tax_id:
                                if payment_terms:
                                    updateQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, ItemSalesTaxRefListID, InvoiceLineTaxCodeRefFullName, Memo, RefNumber, TermsRefFullName) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}','{}','{}', '{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, tax_id, qbd_tax_code, qbd_memo, sanitize(ref_number) if ref_number else '', payment_terms)
                                else:
                                    updateQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, ItemSalesTaxRefListID, InvoiceLineTaxCodeRefFullName, Memo, RefNumber) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}','{}','{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, tax_id, qbd_tax_code, qbd_memo, sanitize(ref_number) if ref_number else '')
                            else:
                                if payment_terms:
                                    updateQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, Memo, RefNumber, TermsRefFullName) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}', '{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, qbd_memo, sanitize(ref_number) if ref_number else '', payment_terms)
                                else:
                                    updateQuery = "Insert into InvoiceLine (InvoiceLineItemRefListID, InvoiceLineQuantity, InvoiceLineRate, InvoiceLineAmount, InvoiceLineDesc, CustomerRefListID, TxnDate, FQSaveToCache, Memo, RefNumber) VALUES ('{}' ,{} ,{} ,{} ,'{}' ,'{}' ,{} ,{}, '{}', '{}')".format(product_quickbooks_id, invoice_line_quantity, invoice_line_rate, invoice_line_amount, invoice_line_desc, customer_quickbooks_id, invoice_line_txn_date, save_to_cache, qbd_memo, sanitize(ref_number) if ref_number else '')
            ...
            # Post-line processing: Create response dict ##############################################################
            if success == 1:
                # Fetch last inserted invoice/bill from QBD ############################################
                if is_vendor_bill:
                    if ref_number:
                        lastInserted = "Select Top 1 TxnID,RefNumber from Bill where RefNumber='{}'".format(sanitize(ref_number))
                    else:
                        lastInserted = "Select Top 1 TxnID,RefNumber from Bill order by TimeCreated desc"
                else:
                    if ref_number:
                        lastInserted = "Select Top 1 TxnId,RefNumber from Invoice where RefNumber='{}'".format(sanitize(ref_number))
                    else:
                        lastInserted = "Select Top 1 TxnId,RefNumber from Invoice order by TimeCreated desc"
                ...
                tpa_invoice_dict['odoo_id'] = rec.get('odoo_id')
                tpa_invoice_dict['quickbooks_id'] = tuple_data[0] if tuple_data else ''
                tpa_invoice_dict['qbd_invoice_ref_no'] = tuple_data[1] if tuple_data else (ref_number or '')
                tpa_invoice_dict['message'] = 'Successfully Created'
                now = datetime.now()
                tpa_invoice_dict['last_modified_date'] = str(now.strftime("%d-%m-%Y %H:%M:%S"))
            ...
            tpa_invoice_list.append(tpa_invoice_dict)
            print(f"\n\n tpa_invoice_list ----- {tpa_invoice_list} \n\n")

        # Finalize - Close cursor and DB connection ##########################
        cursor.close()
        close_connection_to_qbd(con)

        return {"Data": tpa_invoice_list}

     # If no invoice list was present ########################################
    cursor.close()
    close_connection_to_qbd(con)
    return {"Data": []}
# [End] Function for exporting invoices from Odoo to QBD & than QBD to Odoo ##################################################################
