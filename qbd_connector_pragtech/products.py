...
def import_QBD_Products_to_TPA():
    ...
    try:
        data = connect_to_qbd()
        print("11111111111111111111111111111111111111111111",data)
        con = data[0]
        cursor = con.cursor()
        ...
        if request.args.get('inventory_adjustment'):
            print("TO EXECUTE====>>>>>>>>>>>>>>>>",to_execute_account)
            ...
            for rec in req.get('products_list'):
                ...
                if not is_present:
                    ...
                    if rec.get('type') == 'consu':
                        ...
                        qbAssetAccountId = rec.get('AssetAccountRefListID')
                        qbCOGSAccountId = rec.get('COGSAccountRefListID')

                        # print(f"\n\n qbAssetAccountId ----- {qbAssetAccountId} \n\n")
                        # print(f"\n\n qbCOGSAccountId ----- {qbCOGSAccountId} \n\n")
                        ...
                        if qbAssetAccountId and qbCOGSAccountId:
                            # print(f"\n\n if qbAssetAccountId ----- {qbAssetAccountId} and qbCOGSAccountId ----- {qbCOGSAccountId}")
                            if not is_update:
